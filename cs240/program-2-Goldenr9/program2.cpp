...
int main(){
	...
	//testing to make sure table doesn't except more players then seats
	Heap tourney4;
	tourney4.addPlayer(Player("the", 80));
	tourney4.addPlayer(Player("the2", 66665));
	Table highcardTable4(1, 40);
	highcardTable4.addPlayer(tourney4.getPlayer());
	highcardTable4.addPlayer(tourney4.getPlayer());
	assert(highcardTable4.getNumPlayers()==1);
	cerr << "\n\t========================PASS========================\n" << endl;
	//testing to make sure you cant take a player that doesn't exist from a heap
	Heap tourney5;
	
	assert(tourney5.getPlayer().getName().compare("This is not a player")==0);
	cerr << "\n\t========================PASS========================\n" << endl;
	return 0;
}
